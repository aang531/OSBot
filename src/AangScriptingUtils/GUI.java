...
import AangAPI.Misc.GUI.PlaceholderTextField;
import AangAPI.Misc.GUI.NumberFilter;
...
public class GUI extends AangGui {
    ...
    public GUI(AangScriptingUtils script){
        super(script,"AangScriptingUtils");
        setContentPane(root);
        ...
        mouseListener = new BotMouseListener() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (!script.misc.isPointOnScreen(e.getPoint()))
                    return;
                ...
                if( selectingDebugObject ) {
                    ...
                    if( obstacleTypeComboBox.getSelectedIndex() == 0 ){
                        RSObject[] objects = AangUtil.mouse.getObjectsOnCursor();
                        if( objects.length > 0 ){
                        ...
                        Npc[] npcs = AangUtil.mouse.getNpcsOnCursor();
                        if( npcs.length > 0 ){
                    ...
                    Tile t = AangUtil.mouse.getTileOnCursor();
                ...
                //TODO area tiles. make it so when 1 selecting buttun is pressed the others are deactivated
            }
        ...
        p = g -> {
            ...
            if(cursorLocationCheckBox.isSelected()) {
                g.drawString("Mouse x: " + script.mouse.getPosition().x + " y: " + script.mouse.getPosition().y, 5, textYOffset += 20);
            ...
            if(playerLocationCheckBox.isSelected())
                g.drawString("Player pos: "+script.localPlayer().getTile().toString(),5,textYOffset+=20);
            if(hoverEnitiesCheckBox.isSelected()){
                ...
                if(t != null )
                    t.draw(g);
                g.setColor(Color.white);
                //TODO add text field box with basic info about entities
            }
            ...
            switch(tabs.getSelectedIndex()){
                ...
                case PATHMAKER:
                    ...
                    for (int i = 1; i < tilePath.size(); i++) {
                        ...
                        if(AangUtil.misc.isPointOnScreen(a) && AangUtil.misc.isPointOnScreen(b))
                            g.drawLine(a.x,a.y,b.x,b.y);
                    }
                    ...
                    if( obstacleList.getSelectedIndex() != -1 ){
                        ...
                        if( o.type == Obstacle.ObstacleType.OBJECT ){
                            if( o.id != -1 && o.obstacleTile.isValid() ) {
                                RSObject obj = AangUtil.objects.getAt(o.obstacleTile, o.id);
                                g.setColor(Color.yellow);
                                if( obj != null )
                                    obj.drawModel(g);
                                o.obstacleTile.draw(g);
                                if( o.area.contains(AangUtil.localPlayer().getTile()))
                                    g.setColor(new Color(0,255,0,100));
                                else
                                    g.setColor(new Color(255,255,0,100));
                                o.area.fill(g);
                                g.setColor(Color.blue);
                                new Tile(o.area.x,o.area.y,o.area.floor).draw(g);
                                g.setColor(Color.red);
                                new Tile(o.area.x+o.area.width,o.area.y+o.area.height,o.area.floor).draw(g);
                            }
                        }
                    }
                    break;
        ...
        addCurrentTileButton.addActionListener(e -> {
            ...
            final Tile t = AangUtil.script.localPlayer().getTile();
            if( index == -1 ){
                tilePath.add(t);
                lm.addElement(""+t);
            }else{
        ...
        copyToClipboardButtonPath.addActionListener(e1 -> pathToClipBoard());
        ...
        switch(selectedDebugType){
            ...
            case 1:
                if( selectedNPC!= null && selectedNPC.valid() ) {
                    ...
                    text += "Moving: " + selectedNPC.inMotion() + "\n";
            ...
            case 2:
                if( selectedPlayer!= null && selectedPlayer.valid() ) {
                    ...
                    text += "Moving: " + selectedPlayer.inMotion() + "\n";
            ...
            case 3:
                if( selectedTile != null) {
                    ...
                    if (selectedTile.isLoaded()) {
                        text += "Objects On Tile: " + AangUtil.objects.getAt(selectedTile).length + "\n";
    ...
    private void pathToClipBoard(){
        ...
        for( int i = 0; i < tilePath.size(); i++ ) {
            if (i == 0)
                out += "new Tile(" + tilePath.get(i).x + "," + tilePath.get(i).y +  "," + tilePath.get(i).z + ")";
            else
                out += "," + (i % 4 == 3 && i != tilePath.size() - 1 ? "\n\t" : " ") + "new Tile(" + tilePath.get(i).x + "," + tilePath.get(i).y + "," + tilePath.get(i).z + ")";
        }
        ...
        for( int i = 0; i < obstacles.size(); i++ ) {
            ...
            out += "new Obstacle(" + o.id + ", " +
                    (o.type == Obstacle.ObstacleType.OBJECT ? "new Tile(" + o.obstacleTile.x + ", " + o.obstacleTile.y + ", "+o.obstacleTile.z : "") + "), " +
    ...
    private void autoGeneratePath(){
        if( tilePath.size() == 0 ){
            ...
            if (lastTile.distanceTo(AangUtil.localPlayer().getTile()) > 5) {
                DefaultListModel lm = (DefaultListModel) pathList.getModel();
                final Tile t = AangUtil.script.localPlayer().getTile();
                tilePath.add(t);
                lm.addElement("" + t);
    ...
    private void updateObstacle( int index ){
        ...
        if( obstacles.get(index).type == Obstacle.ObstacleType.OBJECT ){
            RSObject o = AangUtil.objects.getAt(obstacles.get(index).obstacleTile, obstacles.get(index).id);
            name = o != null ? o.getName() : "NULL";
        }else{
            Npc npc = AangUtil.npcs.getNearest(obstacles.get(index).id);
        ...
        for( int i = 0; i < script.client.getValidWidgets().length; i++ ){
            if( script.client.getValidWidgets()[i]){
                ...
                for(int j = 0; j < script.widgets.get(i).childCount(); j++ ){
                    ...
                    if( script.widgets.get(i).get(j).childCount() > 0 )
                        for(int k = 0; k < script.widgets.get(i).get(j).childCount(); k++ ){
                            if( script.widgets.get(i,j,k).getText().contains(widgetsFilter.getText()))
                                foundString = true;
